...
contract LimitOrderFacet is ILimitOrder {
    ...
    /**
     * Enumerate all pending limit orders by offset 
     * @param offset the start offset 
     * @param max  The maximum number of Limit orders returned 
     * @return orders  returned limit orders 
     * @return total   the total number of limit orders in protocol
     */
    function getLimitOrders(uint offset, uint max) external view returns (LimitOrderView[] memory orders, uint total) {
        (bytes32[] memory hashes, uint totalOrders) = LibLimitOrder.getLimitOrderHashes(offset, max);

        orders = new LimitOrderView[](hashes.length);
        for (uint i; i < hashes.length; i++) {
            orders[i] = getLimitOrderByHash(hashes[i]);
        }
