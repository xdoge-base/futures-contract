...
contract TradingCoreFacet is ITradingCore, OnlySelf {
    ...
    function lastLongAccFundingFeePerShare(address pairBase) external view override returns (int256 longAccFundingFeePerShare) {
        ...
        if (block.timestamp > ppi.lastFundingFeeTime) {
            int256 fundingFeeR = LibTradingCore.fundingFeeRate(ppi, pairBase);
            longAccFundingFeePerShare += fundingFeeR * (- 1) * int256(block.timestamp - ppi.lastFundingFeeTime);
        }
    ...
    function _updateAccFundingFeePerShare(
        ...
        ppi.longAccFundingFeePerShare += fundingFeeR * (- 1) * int256(block.timestamp - ppi.lastFundingFeeTime);
        ppi.lastFundingFeeTime = block.timestamp;
        return true;
    }
    ...
    function lpUnrealizedPnlUsd(address targetToken) external view override returns (int256 totalUsd, int256 tokenUsd) {
        ...
        // the unrealizedPnlUsd attributed to the token is proportional to the marginUsd of the token
        int256 tokenPnlUsd = totalMarginUsd == 0 ? int256(0) : totalUsd * int256(tokenMarginUsd) / int256(totalMarginUsd);

        return (totalUsd, tokenPnlUsd);
    }
