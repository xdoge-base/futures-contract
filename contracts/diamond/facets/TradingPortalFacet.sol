...
contract TradingPortalFacet is ITradingPortal, OnlySelf {
    ...
    function openMarketTrade(OpenDataInput calldata data) external override {
        ...
        ITrading.PendingTrade memory pt = ITrading.PendingTrade(
            ...
            data.tokenIn, data.qty, data.stopLoss, data.takeProfit, uint128(LibChain.getBlockNumber())
        );
        bytes32 tradeHash = keccak256(abi.encode(pt, ts.salt, "trade", LibChain.getBlockNumber(), block.timestamp));
        ts.salt++;
        ts.pendingTrades[tradeHash] = pt;
        ts.addPendingTradeHash(tradeHash);
