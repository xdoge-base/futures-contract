...
contract TradeXLP is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        require(amount > 0, "TradeXLP: Mint zero amount");
        _mint(to, amount);
    }

    function addFromWhiteList(address account) external onlyRole(ADMIN_ROLE) {
        require(account != address(0), "TradeXLP: Account cannot be 0 address");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal whenNotPaused override {
        ...
        require(from == address(0) || fromWhiteList[from] || to == address(0) || toWhiteList[to],
            "TradeXLP: Non-whitelisted addresses are prohibited from transfer");
    }
